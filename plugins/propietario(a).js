...
switch (true) {     
case isCommand1:
await conn.reply(m.sender, lenguajeEB.smsResP1(), fkontak)
try {
let d = new Date
let date = d.toLocaleDateString('fr', { day: 'numeric', month: 'long', year: 'numeric' })
let database = await fs.readFileSync(`./database.json`)
let creds = await fs.readFileSync(`./KrulBotSession/creds.json`)
await conn.reply(m.sender, lenguajeEB.smsResP2(date), fkontak)
...
case isCommand3:
if (m.fromMe) return
if (isAdmin) return m.reply(lenguajeEB.smsAutoAdmin1())
try {  
await conn.groupParticipantsUpdate(m.chat, [m.sender], "promote")
} catch {
await m.reply(lenguajeEB.smsAutoAdmin2())}        
break
        
case isCommand4:
if (!text) return m.reply(lenguajeEB.smsBioEd1())
if (text.length > 139) return m.reply(lenguajeEB.smsBioEd2())
try {
await conn.updateProfileStatus(text).catch(_ => _)
await conn.reply(m.chat, lenguajeEB.smsBioEd3(), m)
} catch (e) {
reportError(e)
}        
break
...
case isCommand5:
if (!text) return m.reply(lenguajeEB.smsNameEd1())
if (text.length > 25) return m.reply(lenguajeEB.smsNameEd2())
try {
await conn.updateProfileStatus(text).catch(_ => _)
await conn.reply(m.chat, lenguajeEB.smsNameEd3(), m)
} catch (e) {
reportError(e)
}        
break
...
if (!img) return m.reply(lenguajeEB.smsFotoEd1(usedPrefix, command))
await conn.updateProfilePicture(bot, img)
await conn.reply(m.chat, lenguajeEB.smsFotoEd2(), m)
} else return m.reply(lenguajeEB.smsFotoEd3(usedPrefix, command))     
break
...
await conn.reply(m.chat, lenguajeEB.smsBanChE(), m)        
break
        
case isCommand9:
let toUser = `@${m.sender.split("@")[0]}`
let why = lenguajeEB.smsBlockUn1(usedPrefix, command, toUser)
...
aa = ownerNumber + '@s.whatsapp.net'
await conn.reply(m.chat, lenguajeEB.smsBlockUn2(comd, ownerNumber), null, { mentions: [aa] })
return
}}}
switch (true) {		
...
if (res[0]) conn.reply(m.chat, lenguajeEB.smsBlockUn3(comd, useB), m, { mentions: res })
break
...
if(!text && !m.quoted) return conn.reply(m.chat, lenguajeEB.smsRestarU1(), m)
if(isNaN(number)) return conn.reply(m.chat, lenguajeEB.smsRestarU2(), m)
try {
if(text) {
user = number + '@s.whatsapp.net'
} else if(m.quoted.sender) {
...
conn.reply(m.chat, lenguajeEB.smsRestarU3(number), null, { mentions: [user] })
}        
break
...
if (!code1) return m.reply(lenguajeEB.smsJoin1(usedPrefix, command))
try {      
if ( isOwner || m.fromMe) {
await m.reply(lenguajeEB.smsJoin2())
let res1 = await conn.groupAcceptInvite(code1)
await conn.sendMessage(res1, { text: lenguajeEB.smsJoin(user), mentions: (await conn.groupMetadata(`${res1}`)).participants.map(v => v.id) }, [user], { quoted: fkontak })
}} catch (e) {
reportError(e)
}        
break
...
case isCommand12:
if (!text && !m.quoted) return m.reply(lenguajeEB.smsBCMensaje(usedPrefix, command))
...
if (users.length === 0) {
await m.reply(lenguajeEB.smsJBCom4())
return
}
let cc = text ? m : m.quoted ? await m.getQuotedObj() : false || m
let teks = text ? text : cc.text
let content = conn.cMod(m.chat, cc, /bc|broadcast/i.test(teks) ? teks : lenguajeEB.smsJBDifu1() + teks)
for (let id of users) {
await delay(1500)
await conn.copyNForward(id, content, true)
}
let difuUser = `${users.map(v => '❤️ wa.me/' + v.replace(/[^0-9]/g, '') + `?text=${encodeURIComponent(usedPrefix)}estado`).join('\n')}`
let tolUser = users.length * 1.5
let numUser = users.length
await conn.reply(m.chat, lenguajeEB.smsJBDifu2(numUser, difuUser, tolUser).trim(), m)        
break
...
let locale = lenguajeEB.lenguaje()
let dia = d.toLocaleDateString(locale, { weekday: 'long' })
let fecha = d.toLocaleDateString(lenguajeEB.lenguaje(), { day: 'numeric', month: 'numeric', year: 'numeric' })
let mes = d.toLocaleDateString(lenguajeEB.lenguaje(), { month: 'long' })
let a?o = d.toLocaleDateString(lenguajeEB.lenguaje(), { year: 'numeric' })
...
await m.reply(lenguajeEB.smsChatGP1())
//for (let id of groups) {  
//let infoGP = lenguajeEB.smsChatGP2(readMS, dia, mes, a?o, fecha, tiempo)  
//await conn.reply(id, infoGP + teks2, { mentions: usersTag }, { quoted: null })         
//}
for (let i = 0; i < groups.length; i++) {
const id = groups[i];
const infoGP = lenguajeEB.smsChatGP2(readMS, dia, mes, a?o, fecha, tiempo)
const delay = i * 4000 //4 seg
setTimeout(async () => {
await conn.reply(id, infoGP + teks2, { mentions: usersTag }, { quoted: fkontak });
}, delay)}        
let totalGP = groups.length
await m.reply(lenguajeEB.smsChatGP3(totalGP))        
break
...
await conn.reply(m.chat, lenguajeEB.smsBCMensaje2(), m)
let start = new Date().getTime()
let totalPri = 0
for (let user of chats) {
await new Promise(resolve => setTimeout(resolve, 2000)) // 2 segundos
await conn.reply(user, `${lenguajeEB.smsBCbot7()}\n\n` + teks3, null)
totalPri++
if (totalPri >= 500000) { 
break
}}   
...
await m.reply(lenguajeEB.smsBCMensaje3(totalPri, time))        
break
...
//await conn.reply(group, `${lenguajeEB.smsBCbot7()}\n\n` + teks4, { mentions: usersTag2 }, { quoted: m })     
//}
...
await m.reply(`${lenguajeEB.smsBCbot1()}
\`\`\`${lenguajeEB.smsBCbot2()} ? ${totalPrivate2}\`\`\`
\`\`\`${lenguajeEB.smsBCbot3()} ?   ${totalGroups2}\`\`\`
\`\`\`${lenguajeEB.smsBCbot4()} ?   ${total2}\`\`\`\n\n*${lenguajeEB.smsBCbot5()} ${time2}*\n${totalPri2 >= 500000 ? `\n*${lenguajeEB.smsBCbot6()}*` : ''}`)        
break
        
case isCommand16:
try{
await m.reply(lenguajeEB.smsreiniciar()) 
console.log(chalk.bold.magenta('Cargando ???'))
process.send('reset')
} catch (e) {
reportError(e)}  
break
