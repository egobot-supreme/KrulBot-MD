...
} else return m.reply(`${lenguajeEB['smsSetW2']()}`)
break
...
} else return m.reply(`${lenguajeEB['smsSetB2']()}`) 
break
...
if (!img) return m.reply(lenguajeEB.smsGrupoPP())
await conn.updateProfilePicture(m.chat, img).then(_ => m.reply(lenguajeEB.smsGrupoPP2()))
} else return m.reply(lenguajeEB.smsGrupoPP())
break
...
return m.reply(`${lenguajeEB['smsMalused']()}
...
if (!global.db.data.settings[conn.user.jid].restrict) return conn.reply(m.chat, `${lenguajeEB['smsAvisoAG']()}${lenguajeEB['smsSoloOwner']()}`, fkontak, m)
if (args.length >= 1) {
if (/[a-zA-Z]/.test(text)) return conn.reply(m.chat, lenguajeEB.smsAvisoFG() + lenguajeEB.smsInvite1(), m)
text = args.slice(0).join(" ")
} else if (m.quoted && m.quoted.text) {
text = m.quoted.text
} else {
return conn.reply(m.chat, `${lenguajeEB['smsAvisoMG']()}${lenguajeEB.smsInvite2(usedPrefix, command, bot)}`, m);
}  
let NumeroUser = text.replace(/\D/g, '')
user = m.sender.split`@`[0]
let link = 'https://chat.whatsapp.com/' + await conn.groupInviteCode(m.chat)
pp = await conn.profilePictureUrl(m.chat, 'image').catch(_ => null) || egoMenu.getRandom()//'./media/menus/Menu2.jpg' 
...
//await conn.reply(NumeroUser + '@s.whatsapp.net', lenguajeEB.smsInvite3(NumeroUser, user, groupMetadata, link), null, {mentions: [NumeroUser + '@s.whatsapp.net', m.sender]})    
...
await conn.reply(m.chat, lenguajeEB.smsInvite4(NumeroUser), m, {mentions: [NumeroUser + '@s.whatsapp.net', m.sender]})    
break
}} 
