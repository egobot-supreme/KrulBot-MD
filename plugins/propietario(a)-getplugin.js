...
if (!text) throw lenguajeEB.smsPlugin1(usedPrefix, command)
...
await m.reply(lenguajeEB.smsPlugin2(contenidoArchivo, contenido))
return
} catch (err)  {
...
if (!matchingFile) {
return m.reply(lenguajeEB.smsPlugin3(text))
}
...
await m.reply(lenguajeEB.smsPlugin4(filename, fileContent))

} catch (err) {
console.log(lenguajeEB.smsPlugin5(matchingFile, err))
return m.reply(lenguajeEB.smsPlugin5(matchingFile))
}}}
