...
if (!mime) return m.reply(lenguajeEB.smsConURL())
try{ 
media = await q.download()
...
•.̇𖠵⃯🍧 ${isTele ? lenguajeEB.smsConURLERR1() : lenguajeEB.smsConURLERR2()}\n
${lenguajeEB.smsConURL4()}
•.̇𖠵⃯🍧 ${await shortUrl(link)}`.trim()
m.reply(caption) 
} catch (e) {
await m.reply(lenguajeEB['smsMalError3']() + '\n*' + lenguajeEB.smsMensError1() + '*\n*' + usedPrefix + `${lenguajeEB.lenguaje() == 'es' ? 'reporte' : 'report'}` + '* ' + `${lenguajeEB.smsMensError2()} ` + usedPrefix + command)
console.log(`⚠️ ${lenguajeEB['smsMensError2']()} ${usedPrefix + command} ⚠️`)
...
case isCommand3:       
if (!m.quoted) return m.reply(lenguajeEB.smsConVIDEO())
mime = m.quoted.mimetype || ''
if (!/webp|gif/.test(mime)) return m.reply(lenguajeEB.smsConVIDEO2()) 
try{ 
...
case isCommand4:       
if (!m.quoted) return m.reply(lenguajeEB.smsConGIF())
q = m.quoted || m
mime = (q.msg || q).mimetype || ''
if (!/(mp4)/.test(mime)) return m.reply(lenguajeEB.smsConGIF2() + mime)
try{ 
m.reply(global.wait)
media = await q.download()
...
if (!/video|audio/.test(mime)) return m.reply(lenguajeEB.smsConVN())
media = await q.download?.()
if (!media && !/video/.test(mime)) return m.reply(lenguajeEB.smsConVN1())
if (!media && !/audio/.test(mime)) return m.reply(lenguajeEB.smsConVN2())
let audio = await toPTT(media, 'mp4')
if (!audio.data && !/audio/.test(mime)) return m.reply(lenguajeEB.smsConVN3())
if (!audio.data && !/video/.test(mime)) return m.reply(lenguajeEB.smsConVN4())
try{
...
//m.reply(e + '')
text = args.join(' ')
if (!text) return m.reply(lenguajeEB.smsTradc1() + usedPrefix + command + lenguajeEB.smsTradc2() + usedPrefix + command + ' ' + lenguajeEB.smsTradc3() + lenguajeEB.smsTradc4())
