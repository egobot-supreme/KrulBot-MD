//ESTOS DISEÑOS PARA MENSAJES SON COMPATIBLES CON EGOBOT, SIN EMBARGO ALGUNOS NO SE VEN EN LA VERSIÓN DE WHATSAPP WEB
...
let sgh = md
let sgc = nnn
let ego = 'https://i.postimg.cc/QxrChCTy/Picsart-23-09-27-13-06-00-148.jpg'
//let logo = 'https://i.postimg.cc/y8twCgJJ/dad0470e02d98e88269232493090ad35.jpg'
let pp = await conn.profilePictureUrl(who).catch(_ => ego)
...
//global.enlace2 = { contextInfo: { externalAdReply: { showAdAttribution: true, mediaUrl: yt, mediaType: 'VIDEO', description: '', title: wm, body: '🚀 𝗠𝗲𝗴𝗮 𝗘𝗴𝗼𝗕𝗼𝘁-𝗠𝗗', thumbnailUrl: await(await fetch(img)).buffer(), sourceUrl: yt }}}
...
//global.fvn2 = { key: {  fromMe: false, participant: `0@s.whatsapp.net`, ...(false ? { remoteJid: "5493873687620-5493873452654@g.us" } : {}) }, message: { "audioMessage": { "mimetype":"audio/ogg; codecs=opus", "seconds": "99569", "ptt": "true"   }}}  
//SIMULADOR DE AUDIO ENVIADO 
	
//global.twa = {key: {participant: "0@s.whatsapp.net", "remoteJid": "0@s.whatsapp.net"}, "message": {"groupInviteMessage": {"groupJid": "5493873687620-5493873452654@g.us", "inviteCode": "m", "groupName": "P", "caption": wm, 'jpegThumbnail': fs.readFileSync('./media/menus/Menu3.jpg')}}}
...
/*let adReply = { fileLength: fsizedoc, seconds: fsizedoc, contextInfo: { forwardingScore: fsizedoc, externalAdReply: { showAdAttribution: true, title: wm, body: '🔥 ' + username, mediaUrl: ig, description: 'Hola', previewType: 'PHOTO', thumbnail: await(await fetch(egoMenu.getRandom())).buffer(), sourceUrl: redesMenu.getRandom() }}}	
...
function ucapan() {
  ...
  let res = "🧐🍷 𝘽𝙞𝙚𝙣𝙫𝙚𝙣𝙞𝙙𝙤/𝙖 𝙀𝙨𝙩𝙞𝙢𝙖𝙙𝙤/𝙖 🔥"
  if (time >= 4) {
    res = "🧐🍷 𝘽𝙪𝙚𝙣𝙤𝙨 𝙙𝙞𝙖𝙨 𝙚𝙨𝙩𝙞𝙢𝙖𝙙𝙤/𝙖 ☕"
  }
  if (time >= 11) {
    res = "🧐🍷 𝘽𝙪𝙚𝙣𝙖𝙨 𝙩𝙖𝙧𝙙𝙚𝙨 𝙚𝙨𝙩𝙞𝙢𝙖𝙙𝙤/𝙖 💪🏻"
  }
  if (time >= 15) {
    res = "🧐🍷 𝘽𝙪𝙚𝙣𝙖𝙨 𝙩𝙖𝙧𝙙𝙚𝙨 𝙚𝙨𝙩𝙞𝙢𝙖𝙙𝙤/𝙖 🍹"
  }
  if (time >= 17) {
    res = "🧐🍷 𝘽𝙪𝙚𝙣𝙖𝙨 𝙣𝙤𝙘𝙝𝙚𝙨 𝙚𝙨𝙩𝙞𝙢𝙖𝙙𝙤/𝙖 💤"
  }
  return res
} 
