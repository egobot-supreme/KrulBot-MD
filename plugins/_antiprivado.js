...
user.warn += 1
if (!(user.warn >= 6)) await this.sendMessage(m.chat, { text: `*${lenguajeEB['smsCreA']()}* *@${m.sender.split`@`[0]}*, ${lenguajeEB['smsprivado']()}\n${nn}`, mentions: [m.sender] }, { quoted: fkontak })
await conn.groupParticipantsUpdate(m.chat, [m.sender], 'banchat')}
//await m.reply(`*${lenguajeEB['smsCreA']()}* *@${m.sender.split`@`[0]}*, ${lenguajeEB['smsprivado']()}\n${nn}`,mentions: [m.sender] }, { quoted: fkontak })
if (user.warn >= 6) {
user.warn = 0 
await conn.groupParticipantsUpdate(m.chat, [m.sender], 'banchat')
}
