...
global.authFile = `KrulBotSession`;
...
console.log(chalk.bold.greenBright(lenguajeEB.languageSave()))}
function promptLoop() {
console.log(`
╔╤═══⟬🍁•ೋ°ৡৢ͜͡  ❮❤️❯ 𖥨֗°ೋ•🍁⟭═══╗            
...
╚╧═══⟬🍁•ೋ°ৡৢ͜͡  ❮❤️❯ 𖥨֗°ೋ•🍁⟭═══╝        
❮💜❯➣ ${chalk.bold.yellowBright('If you choose to skip, you will not have the opportunity to change the language later through the console.')}
❮💜❯➣ ${chalk.bold.yellowBright('Si elige omitir, no tendrá oportunidad de cambiar el idioma más tarde a través de la consola.')}\n
▼ ▼ ▼`.trim())
...
configContent = configContent.replace('global.lenguajeEB = es', `global.lenguajeEB = ${selectedLanguage}`)
fs.writeFileSync(configPath, configContent, 'utf8')
console.log(chalk.bold.cyanBright(lenguajeEB.languageRegister(selectedLanguage)))
...
/*console.log('👑 Escriba el número que será propietario, ejemplo: +593 99 000 0000')
console.log('👑 Si piensa agregar varios números separé por "," ❤️ ejemplo: +593 99 000 0000, +52 1 000 000 0000, +598 00 000 000')
...
if (cleanedNumbers.length === 1) {
console.log(`\n❤️ Se  ha agregado el número "+${cleanedNumbers[0]}" como propietario.`)
} else {
console.log(`\n❤️ Se han agregado los números "+${cleanedNumbers.join(', ')}" como propietarios.`)
}} else {
console.log('\n❤️ 'Se ha omitido la adición de número/s como propietario/s.')
}*/
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "KrulJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (connection == 'open') {
console.log(chalk.bold.greenBright(lenguajeEB['smsConexion']()))}
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
if (connection === 'close') {
if (reason === DisconnectReason.badSession) {
console.log(chalk.bold.cyanBright(lenguajeEB['smsConexionOFF']()))
} else if (reason === DisconnectReason.connectionClosed) {
console.log(chalk.bold.magentaBright(lenguajeEB['smsConexioncerrar']()))
process.send('reset')
} else if (reason === DisconnectReason.connectionLost) {
console.log(chalk.bold.blueBright(lenguajeEB['smsConexionperdida']()))
process.send('reset')
} else if (reason === DisconnectReason.connectionReplaced) {
console.log(chalk.bold.yellowBright(lenguajeEB['smsConexionreem']()))
} else if (reason === DisconnectReason.loggedOut) {
console.log(chalk.bold.redBright(lenguajeEB['smsConexionOFF']()))
} else if (reason === DisconnectReason.restartRequired) {
console.log(chalk.bold.cyanBright(lenguajeEB['smsConexionreinicio']()))
} else if (reason === DisconnectReason.timedOut) {
console.log(chalk.bold.yellowBright(lenguajeEB['smsConexiontiem']()))
process.send('reset')
} else {
console.log(chalk.bold.redBright(lenguajeEB['smsConexiondescon'](reason, connection)))
}}
}
...
let prekey = []
let directorio = readdirSync("./KrulBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./KrulBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./KrulJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./KrulJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./KrulJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') || fileInDir.startsWith('app-') || fileInDir.startsWith('session-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./KrulJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./KrulBotSession/', './KrulJadiBot/']
...
setInterval(async () => {
await clearTmp()        
console.log(chalk.bold.cyanBright(lenguajeEB.smsClearTmp()))}, 1000 * 60 * 4) // 4 min 

setInterval(async () => {
await purgeSession()
console.log(chalk.bold.cyanBright(lenguajeEB.smspurgeSession()))}, 1000 * 60 * 10) // 10 min
...
setInterval(async () => {
await purgeOldFiles()
console.log(chalk.bold.cyanBright(lenguajeEB.smspurgeOldFiles()))}, 1000 * 60 * 10)

_quickTest().then(() => conn.logger.info(chalk.bold(lenguajeEB['smsCargando']().trim()))).catch(console.error)

let file = fileURLToPath(import.meta.url)
watchFile(file, () => {
unwatchFile(file)
console.log(chalk.bold.greenBright(lenguajeEB['smsMainBot']().trim()))
