...
const smsMalused2 = () => { return `${lenguajeEB['smsAvisoMG']()}🐈 *YOU MUST USE THE COMMAND AS IN THIS EXAMPLE:*\n`}
const smsMalused3 = () => { return `${lenguajeEB['smsAvisoMG']()}🐈 *REPLY TO A MESSAGE WITH THE COMMAND OR USE THIS EXAMPLE:*\n`}

//Error
const smsMalError = () => { return `${lenguajeEB['smsAvisoFG']()}\`\`\`AN UNEXPECTED ERROR OCCURRED.\`\`\``}
const smsMalError2 = () => { return `${lenguajeEB['smsAvisoFG']()}\`\`\`AN INCONVENIENCE ARISED.\`\`\`\n`}
const smsMalError3 = () => { return `${lenguajeEB['smsAvisoFG']()}\`\`\`SOMETHING WENT WRONG, REPORT THIS COMMAND USING:\`\`\`\n`}
...
//convertidores.js
const smsToimg = () => { return lenguajeEB.smsAvisoMG() + '*RESPOND TO A STICKER TO CONVERT TO IMAGE*'} 

//convertidores.js
const smsConURL = () => { return lenguajeEB.smsAvisoMG() + '*RESPOND TO AN IMAGE TO CONVERT TO A LINK*'} 
...
//convertidores.js
const smsConVIDEO = () => { return lenguajeEB.smsAvisoMG() + '*RESPOND TO MOTION STICKER TO CONVERT TO VIDEO*'} 
const smsConVIDEO2 = () => { return lenguajeEB.smsAvisoMG() + '*ONLY STICKER IN MOTION TO CONVERT TO VIDEO*'} 
const smsConVIDEO3 = () => { return lenguajeEB.smsAvisoEG() + '*✓ HERE YOU HAVE YOUR VIDEO!!*'} 

//convertidores.js
const smsConGIF = () => { return lenguajeEB.smsAvisoMG() + '*REPLY TO A VIDEO TO CONVERT TO A GIF WITH AUDIO*'} 
const smsConGIF2 = () => { return lenguajeEB.smsAvisoMG() + '*RESPOND TO A VIDEO. THE TYPE OF FILES USED IS NOT CORRECT* '} 
const smsConGIF3 = () => { return lenguajeEB.smsAvisoEG() + '*GIF WITH AUDIO (Open the gif to hear the audio)*'} 

//convertidores.js
const smsConVN = () => { return lenguajeEB.smsAvisoMG() + '*REPLY TO A VIDEO OR AUDIO TO CONVERT TO A VOICE NOTE*'} 
const smsConVN1 = () => { return lenguajeEB.smsAvisoFG() + '*SOMETHING WRONG WITH THE VIDEO, TRY AGAIN*'} 
const smsConVN2 = () => { return lenguajeEB.smsAvisoFG() + '*SOMETHING WRONG WITH THE AUDIO, TRY AGAIN*'} 
const smsConVN3 = () => { return lenguajeEB.smsAvisoFG() + '*SOMETHING WENT WRONG WHEN TRYING TO CONVERT YOUR AUDIO TO VOICE MEMO*'}
const smsConVN4 = () => { return lenguajeEB.smsAvisoFG() + '*SOMETHING WENT WRONG WHEN TRYING TO CONVERT YOUR VIDEO TO VOICE MEMO*'} 

//convertidores.js
const smsTradc1 = () => { return lenguajeEB.smsAvisoMG() + '*REPLY TO A TEXT MESSAGE OR TYPE THE CODE AND TEXT TO CONVERT TO AUDIO, EXAMPLE:*\n'}
const smsTradc2 = () => { return ' (code) (text)\n'} 
const smsTradc3 = () => { return lenguajeEB.lenguaje() + ' This text will be audio\n\n'}
...
//grupos-admins.js
const smsSetW = () => { return `${lenguajeEB['smsAvisoEG']()}*GROUP WELCOME HAS BEEN SET UP*`}
const smsSetW2 = () => { return `${lenguajeEB['smsAvisoIIG']()}🙌 *_WRITE THE WELCOME MESSAGE_*\n*_OPTIONAL YOU CAN USE WHAT IS WITH "@" TO ADD MORE INFORMATION:_*\n\n*⚡ @user (Mention to the user)*\n*⚡ @subject (Group name)*\n*⚡ @desc (Group description)*\n\n*REMEMBER THAT THE "@" ARE OPTIONAL*`}

//grupos-admins.js
const smsSetB = () => { return `${lenguajeEB['smsAvisoEG']()}*THE FAREWELL OF THE GROUP HAS BEEN CONFIGURED*`}
const smsSetB2 = () => { return `${lenguajeEB['smsAvisoIIG']()}🙌 *_WRITE THE FAREWELL MESSAGE_*\n*_OPTIONAL YOU CAN USE WHAT IS WITH "@" TO ADD MORE INFORMATION:_*\n\n*⚡ @user (Mention to the user)*\n\n*REMEMBER THAT THE "@" IS OPTIONAL*`}

//grupos-admins.js
const smsDest = () => { return `${lenguajeEB['smsAvisoEG']()}*GROUP DESCRIPTION HAS BEEN CONFIGURED*`}

//grupos-admins.js
const smsNam1 = () => { return `${lenguajeEB['smsAvisoEG']()}*THE GROUP NAME HAS BEEN SET*`}
const smsNam2 = () => { return `${lenguajeEB['smsAvisoMG']()}*🙌 WRITE THE NEW GROUP NAME*`}
const smsNam3 = () => { return `${lenguajeEB['smsAvisoFG']()}*THE NAME OF THE GROUP MUST NOT HAVE MORE THAN 25 CHARACTERS*`}

//grupos-admins.js
const smsGrupoPP = () => { return `${lenguajeEB['smsAvisoMG']()}*RESPOND TO AN PICTURE TO CHANGE THE GROUP PHOTO*`}
const smsGrupoPP2 = () => { return `${lenguajeEB['smsAvisoEG']()}*🪄 THE IMAGE OF THE GROUP WAS UPDATED!!*`}

//grupos-admins.js
const smsRestGp = () => { return `${lenguajeEB['smsAvisoEG']()}*THE GROUP LINK HAS BEEN RESETTED*`}

//grupos-admins.js
const smskick1 = () => { return `${lenguajeEB['smsAvisoAG']()}*TAG THE PERSON OR REPLY TO THE MESSAGE OF THE PERSON YOU WANT TO REMOVE*\n\n*EXAMPLE: `}
...
const smsGrupoClose = () => { return `${lenguajeEB['smsAvisoEG']()}*CLOSED GROUP, ONLY ADMINS CAN WRITE!!*`}
...
//propietario(a.js
const smsPropban1 = (usedPrefix, command, bot) => { return `${lenguajeEB['smsAvisoMG']()}*TAG SOMEONE OR REPLY TO THE USER'S MESSAGE OR WRITE THE NUMBER YOU WANT TO BAN FROM THE COMMANDS*\n\n*EXAMPLE:*\n*${usedPrefix + command} @${bot}*`}
const smsPropban2 = (bot) => { return `${lenguajeEB['smsAvisoFG']()}*@${bot} CANNOT BE BANNED WITH THIS COMMAND* 😹`}
const smsPropban3 = (ownerNumber) => { return `${lenguajeEB.smsAvisoIIG()}😳 *I CAN'T BAN THE OWNER @${ownerNumber} OF ${packname}*`}
const smsPropban4 = (number) => { return `${lenguajeEB.smsAvisoIIG()}*IT IS NOT NECESSARY TO BAN AGAIN @${number} YES IT ALREADY IS* 😊`}
const smsPropban5 = () => { return `${lenguajeEB['smsAvisoEG']()}*USER SUCCESSFULLY BANNED* 🙀`}
const smsPropban6 = (number, usr) => { return `${lenguajeEB.smsAvisoAG()}*@${number} YOU ARE BANNED BY @${usr} YOU CANNOT USE THE COMMANDS UNTIL SOMEONE REVERSES THE BAN* 😿`}
const smsPropban7 = (usedPrefix, command, number) => { return `${lenguajeEB['smsAvisoFG']()}*AN ERROR CAME UP, IT MAY BE THAT THE USER IS NOT IN MY DATABASE TRY TO WRITE ${usedPrefix + command} ${number}*\n\`\`\`IF THE ERROR CONTINUES REPORT THIS COMMAND\`\`\``}

//owner-unbanuser.js
const smsPropdesban1 = (usedPrefix, command, bot) => { return `${lenguajeEB['smsAvisoMG']()}*TAG SOMEONE OR REPLY TO THE USER'S MESSAGE OR WRITE THE NUMBER YOU WANT TO UNBAN FROM COMMANDS*\n\n*EXAMPLE:*\n*${usedPrefix + command} @${bot}*`}
const smsPropdesban2 = (number) => { return `${lenguajeEB.smsAvisoIIG()}*NO NEED TO UNBAN AGAIN @${number} YES IT ALREADY IS* 😊`}
const smsPropdesban3 = () => { return `${lenguajeEB['smsAvisoEG']()}*USER UNBANED SUCCESSFULLY* 💙`}
const smsPropdesban4 = (number, usr) => { return `${lenguajeEB.smsAvisoAG()}*@${number} YOU ARE UNBANED BY @${usr} YOU CAN NOW USE THE COMMANDS!!* 🪄`}
const smsPropdesban5 = (usedPrefix, command, number) => { return `${lenguajeEB['smsAvisoFG']()}*AN ERROR CAME UP, IT MAY BE THAT THE USER IS NOT IN MY DATABASE TRY TO WRITE ${usedPrefix + command} ${number}*\n\`\`\`IF THE ERROR CONTINUES REPORT THIS COMMAND\`\`\``}
...
//owner-unbanchat.js
const smsUnbanCH1 = () => { return lenguajeEB['smsAvisoFG']() + '🗂️ *THIS CHAT IS NOT REGISTERED IN THE DATABASE*'}
const smsUnbanCH2 = () => { return lenguajeEB.smsAvisoAG() + '🌹 *THIS CHAT IS NOT BANNED!!*'}
const smsUnbanCH3 = () => { return lenguajeEB['smsAvisoEG']() + '🪄 *THIS CHAT WAS UNBANED, YOU CAN NOW USE THE COMMANDS!!*'}
...
const smsBlockUn2 = (comd, ownerNumber) => { return lenguajeEB.smsAvisoAG() + `*CAN'T ${comd} TO THE OWNER @${ownerNumber}*`}
const smsBlockUn3 = (comd, useB) => { return `*IT HAS BEEN ${comd} TO ${useB} WITH ÉXITO*`}

//propietario(a).js
const smsRestarU1 = () => { return lenguajeEB['smsAvisoMG']() + '*LABEL THE USER, WRITE THEIR NUMBER OR RESPOND TO THE MESSAGE TO RESTART DATA*'}
const smsRestarU2 = () => { return lenguajeEB['smsAvisoFG']() + '*THE NUMBER YOU ENTERED IS INVALID TO RESET THE DATA*'}
const smsRestarU3 = (number) => { return lenguajeEB['smsAvisoEG']() + `*RESETS TO @${number} FROM THE DATABASE*`}

//propietario(a).js
const smsJoin1 = (usedPrefix, command) => { return lenguajeEB['smsAvisoMG']() + `*ENTER THE LINK OF A GROUP*\n*EXAMPLE:*\n*${usedPrefix + command}* ${nna}`}
const smsJoin2 = () => { return lenguajeEB['smsAvisoEG']() + `${packname}\n*HE HAS JOINED THE GROUP ✅*`}

//info.js
const smsReportEB1 = (usedPrefix, command) => { return lenguajeEB['smsAvisoMG']() + `*WRITE A REPORT*\n*EXAMPLE:*\n\n*${usedPrefix + command}* _The command ${usedPrefix}play it does not work._`}
const smsReportEB2 = () => { return lenguajeEB['smsAvisoFG']() + `✨ *MINIMUM 10 CHARACTERS TO MAKE THE REPORT*`}
const smsReportEB3 = () => { return lenguajeEB['smsAvisoFG']() + `😼 *MAXIMUM 1000 CHARACTERS TO MAKE THE REPORT*`}
...
*⎔ Message:*
*» ${text}*`.trim()}
const smsReportEB5 = () => { return lenguajeEB['smsAvisoEG']() + `*THE REPORT HAS BEEN SENT TO THE DEVELOPERS, IF NECESSARY YOU WILL HAVE A RESPONSE*`}
...
const smsSoloOwnerJB = () => { return `${lenguajeEB['smsAvisoAG']()}*THIS COMMAND IS DISABLED BY MY OWNER*`}
const smsJBPrincipal = () => { return `${lenguajeEB['smsAvisoAG']()}🔵 *TO BE A SUB BOT GO TO THE MAIN NUMBER*\n*ღ Enter the following link:*\n`}
const smsJBConexion = () => { return `${lenguajeEB['smsAvisoFG']()}🟡 *THE CONNECTION HAS BEEN CLOSED UNEXPECTEDLY, WE WILL TRY TO RECONNECT...*`}
const smsJBConexionClose = () => { return `${lenguajeEB['smsAvisoFG']()}🔴 *THE CONNECTION HAS BEEN CLOSED, YOU MUST CONNECT MANUALLY USING THE #serbot COMMAND AND RESCAN THE NEW QR CODE${lenguajeEB['smsJBConexionTrue3']()}* *WHICH WAS SENT THE FIRST TIME A SUB BOT WAS MADE*`}
const smsJBConexionTrue = () => { return `${lenguajeEB['smsAvisoEG']()}🟢 *SUCCESSFUL CONNECTION!!!*`}
const smsJBConexionTrue2 = () => { return `${lenguajeEB['smsAvisoEG']()}🟢 *CONNECTION SUCCESSFUL!!! YOU CAN CONNECT USING:*`}
const smsJBConexionTrue3 = () => { return `.`}
const smsJBCargando = (usedPrefix) => { return `${lenguajeEB['smsAvisoIIG']()}⚪ *IS CONNECTED!! PLEASE WAIT MESSAGES ARE LOADING...*\n\n♻️ *AVAILABLE OPTIONS:*\n*» ${usedPrefix}pausesb _(Stop Sub Bot Feature)_*\n*» ${usedPrefix}delsession _(Delete all traces of Sub Bot)_*\n*» ${usedPrefix}jadibot _(New QR code or Login if you are already a Sub Bot)_*`}

//jadibot
const smsJBDel = () => { return `${lenguajeEB['smsAvisoAG']()}*USE THIS COMMAND TO THE MAIN BOT*`}
const smsJBAdios = () => { return `${lenguajeEB['smsAvisoEG']()}*I WILL MISS YOU ${global.packname} BYE!! 🥹*`}
const smsJBCerrarS = () => { return `${lenguajeEB['smsAvisoEG']()}*YOU HAVE LOGGED OUT AND DELETED ALL TRACES*`}
...
//jadibot
const smsJBCom1 = () => { return lenguajeEB['smsAvisoAG']() + `*IF YOU ARE NOT A SUB BOT, CONTACT THE MAIN NUMBER TO BECOME A SUB BOT*`}
const smsJBCom2 = () => { return lenguajeEB['smsAvisoRG']() + `*YOU HAVE CLOSED/PAUSED THE SESSION.*`}
...
//rpg-verificar.js
const smsVerify0 = (usedPrefix) => { return `${lenguajeEB['smsAvisoIIG']()}*YOU ARE ALREADY REGISTERED!!*\n*IF YOU WANT TO UNREGISTER YOUR REGISTRATION USE THIS COMMAND*\n*${usedPrefix}unreg serial number*\n\n*IF YOU DON'T REMEMBER YOUR SERIAL NUMBER USE THIS COMMAND*\n*${usedPrefix}myns`}
const smsVerify1 = (usedPrefix, command) => { return `${lenguajeEB['smsAvisoIIG']()}*ENTER YOUR NAME AND AGE TO BE REGISTERED*\n*EXAMPLE*\n\n${usedPrefix + command} GataBot.18`}
const smsVerify2 = () => { return `${lenguajeEB['smsAvisoAG']()}*ENTER YOUR NAME*`}
const smsVerify3 = () => { return `${lenguajeEB['smsAvisoAG']()}*ENTER YOUR AGE*`}
const smsVerify4 = () => { return `${lenguajeEB['smsAvisoAG']()}*YOU ARE VERY OLD*`}
const smsVerify5 = () => { return `${lenguajeEB['smsAvisoAG']()}*YOU ARE VERY MINOR*`}
const smsVerify6 = () => { return `${lenguajeEB['smsAvisoAG']()}*WRITE A SHORTER NAME*`}
...
//_antilink.js
const smsAdwa = () => { return `${global.lenguajeEB['smsAvisoEG']()}*AS IT IS ADMIN IT WILL NOT BE DELETED*`}
const smsEnlaceWat = () => { return `${lenguajeEB['smsAvisoAG']()}*A WHATSAPP LINK WAS DETECTED!!*\n\n*YOU WILL BE DELETED*`}
const smsWaMismoEnlace = () => { return '*THIS LINK IS FROM THIS GROUP, SOLELY FOR THAT, IT WILL NOT BE DELETED*'}

//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeEB['smsAvisoAG']()}*A LINK CONTAINING HTTPS WAS DETECTED!!*\n\n*YOU WILL BE DELETED*`}
...
const smsCont7 = () => { return `${global.lenguajeEB['smsAvisoAG']()}*HAVE NO DIAMONDS!! 💎 YOU CAN GO TO THE STORE WITH THE COMMAND*`}
const smsCont8 = () => { return ` *DIAMONDS 💎 USED*`}
const smsCont9 = () => { return `${global.lenguajeEB['smsAvisoAG']()}*NEED THE LEVEL ➡️*`}
...
const smsSP6 = () => { return `${lenguajeEB['smsAvisoFG']()}*NO RESULTS FOUND. TRY ANOTHER NAME OR LINK*`} 
