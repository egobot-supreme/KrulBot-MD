...
const smsMalused3 = () => { return `${lenguajeEB['smsAvisoMG']()}🐈 *قم بالرد على رسالة بالأمر أو استخدم هذا المثال:*\n`}
...
//convertidores.js
const smsToimg = () => { return lenguajeEB.smsAvisoMG() + '*الرد على ملصق للتحويل إلى صورة*'} 
...
//convertidores.js
const smsConVIDEO = () => { return lenguajeEB.smsAvisoMG() + '*الرد على ملصق الحركة للتحويل إلى الفيديو*'} 
const smsConVIDEO2 = () => { return lenguajeEB.smsAvisoMG() + '*ملصق في الحركة فقط للتحويل إلى فيديو*'} 
const smsConVIDEO3 = () => { return lenguajeEB.smsAvisoEG() + '*✓ هنا لديك الفيديو الخاص بك !!*'} 

//convertidores.js
const smsConGIF = () => { return lenguajeEB.smsAvisoMG() + '*رد على مقطع فيديو للتحويل إلى ملف GIF بالصوت*'} 
const smsConGIF2 = () => { return lenguajeEB.smsAvisoMG() + '*الرد على الفيديو. نوع الملفات المستخدمة غير صحيح* '} 
const smsConGIF3 = () => { return lenguajeEB.smsAvisoEG() + '*GIF مع الصوت (افتح gif لسماع الصوت)*'} 
...
const smsConVN3 = () => { return lenguajeEB.smsAvisoFG() + '*حدث خطأ ما عند محاولة تحويل الصوت إلى المذكرة الصوتية*'}
const smsConVN4 = () => { return lenguajeEB.smsAvisoFG() + '*حدث خطأ ما عند محاولة تحويل الفيديو الخاص بك إلى مذكرة صوتية*'} 

//convertidores.js
const smsTradc1 = () => { return lenguajeEB.smsAvisoMG() + '*رد على رسالة نصية أو اكتب الكود والنص للتحويل إلى الصوت ، على سبيل المثال:*\n'}
const smsTradc2 = () => { return ' (كود) (نص)\n'} 
const smsTradc3 = () => { return lenguajeEB.lenguaje() + ' سيكون هذا النص صوتيًا\n\n'}
...
//grupos-admins.js
const smsSetB = () => { return `${lenguajeEB['smsAvisoEG']()}*تم تكوين وداع المجموعة*`}
...
const smsNam3 = () => { return `${lenguajeEB['smsAvisoFG']()}*يجب ألا يحتوي اسم المجموعة على أكثر من 25 حرفًا*`}
...
const smsGrupoClose = () => { return `${lenguajeEB['smsAvisoEG']()}*مجموعة مغلقة ، يمكن للمشرفين فقط الكتابة !!*`}
...
const smsPropban3 = (ownerNumber) => { return `${lenguajeEB.smsAvisoIIG()}😳 *لا يمكنني منع المالك @${ownerNumber} من ${packname}*`}
...
//owner-unbanchat.js
const smsUnbanCH1 = () => { return lenguajeEB['smsAvisoFG']() + '🗂️ *هذه المحادثة غير مسجلة في قاعدة البيانات*'}
const smsUnbanCH2 = () => { return lenguajeEB.smsAvisoAG() + '🌹 *هذه المحادثة غير محظورة !!*'}
...
const smsBlockUn2 = (comd, ownerNumber) => { return lenguajeEB.smsAvisoAG() + `*لا أستطيع ${comd} للمالك @${ownerNumber}*`}
...
const smsReportEB3 = () => { return lenguajeEB['smsAvisoFG']() + `😼 *1000 حرف كحد أقصى لإعداد التقرير*`}
...
const smsSoloOwnerJB = () => { return `${lenguajeEB['smsAvisoAG']()}*تم تعطيل هذا الأمر من قبل المالك*`}
...
const smsJBConexionClose = () => { return `${lenguajeEB['smsAvisoFG']()}🔴 *تم إغلاق الاتصال ، يجب عليك الاتصال يدويًا باستخدام الأمر #jadibot وإعادة إنشاء رمز الاستجابة السريعة الجديد${lenguajeEB['smsJBConexionTrue3']()}* *الذي تم إرساله لأول مرة تم إنشاء بوت فرعي*`}
const smsJBConexionTrue = () => { return `${lenguajeEB['smsAvisoEG']()}🟢 *اتصال ناجح !!!*`}
const smsJBConexionTrue2 = () => { return `${lenguajeEB['smsAvisoEG']()}🟢 *تم الاتصال بنجاح!!! يمكنك الاتصال باستخدام:*`}
...
const smsEnlaceWat = () => { return `${lenguajeEB['smsAvisoAG']()}*تم اكتشاف رابط WHATSAPP !!*\n\n*سيتم حذفك*`}
const smsWaMismoEnlace = () => { return '*هذا الرابط من هذه المجموعة ، فقط لأنه لن يتم حذفه*'}

//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeEB['smsAvisoAG']()}*تم اكتشاف رابط يحتوي على HTTPS !!*\n\n*سيتم حذفك*`}
...
const smsCont9 = () => { return `${global.lenguajeEB['smsAvisoAG']()}*بحاجة إلى المستوى ➡️*`}
...
const smsSP6 = () => { return `${lenguajeEB['smsAvisoFG']()}*لم يتم العثور على نتائج. جرب اسمًا أو رابطًا آخر*`} 
